...
mod todo_app {
    ...
    mod tests {
        ...
        fn default_works() {
            ...
            assert_eq!(todo_app.get_items().len(), 0);
        }

        #[ink::test]
        fn add_item_works() {
            let mut todo_app = TodoApp::new();
            todo_app.add_item("write tests".into());
            assert_eq!(todo_app.get_items().len(), 1);
            assert_eq!(todo_app.get_items()[0].description, "write tests");
            assert!(!todo_app.get_items()[0].completed);
        }
        ...
        fn clear_completed_works() {
            let mut todo_app = TodoApp::new();
            todo_app.add_item("write tests".into());
            todo_app.add_item("deploy contract".into());
            todo_app.mark_completed(0);
            todo_app.clear_completed();
            assert_eq!(todo_app.get_items().len(), 1);
            assert_eq!(todo_app.get_items()[0].description, "deploy contract");
        }
    }
}
